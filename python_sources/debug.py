...
def deep_predict(input, classifier):
    ...
    def predict(filename, classeattendue, fichier, numcri, logpred):
        ...
        fin = time.time()
        print("\t TEMPS ECOULE : {}ms".format((fin-deb)*1000))
        ...
        sc = ";"
        ligne_debut = classeattendue + sc + str(
            numtest) + "_" + snbepoch + "_" + affitest + sc + f + sc + fichier + sc + numcri + sc + class_pred + sc + str(
            rconf) + sc + sbon
        #
        ligne_fin = ""
        confat = 100
    ...
    for j in range(0, finboucle):
        ...
        for esp in labels:
            if tabnbtests[esp] > 0:
                ...
                ecrirelog(
                    "      " + esp + "  :  " + str(tabnbbons[esp]) + " / " + str(tabnbtests[esp]) + "   --> " + str(
                        tabscore[esp]) + " %")
        if nbespreelles > 0:
    ...
    if FAIREPHASE4:
        ...
        for k in range(1, 4):
            lentetepredw = lentetepredw + sc + "Cri" + sc + "Pred. " + str(k) + sc + "Conf. " + str(
                k) + sc + "ok" + str(k)
    ...
    if len(tcor2) > 0:
        ...
        if aumoins1deuxiemeregr:
            ...
            ecrirelog(
                "aucun deuxieme regroupement de classe trouve dans les classes attendues : donc pas de bilan deuxieme regroupement")
    else:
